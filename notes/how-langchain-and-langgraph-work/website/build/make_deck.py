...
evolution of real langchain/langgraph code running at Beanline Coffee. Everything
else in the scaffold passes through unchanged. Output is langchain.qmd, which
...
DECK = HERE.parent  # notes/how-langchain-and-langgraph-work/website
ASSETS = DECK / "assets"
...
def main() -> None:
    ...
    for line in scaffold:
        ...
        if m:
            ...
            out.append(line)
    (DECK / "langchain.qmd").write_text("".join(out))
    print(f"wrote langchain.qmd: {n_fb} flipbooks, {n_frames} figure slides")

    if "--render" in sys.argv:
        proc = subprocess.run(
            ["quarto", "render", "langchain.qmd", "--to", "revealjs"],
            cwd=DECK,
            capture_output=True,
            text=True,
        )
