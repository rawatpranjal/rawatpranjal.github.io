...
evolution of a real toy agentic harness. Everything else in the scaffold passes through
unchanged. Output is harness.qmd, which quarto renders to the folder deck.
...
DECK = HERE.parent  # notes/how-rag-works/website
ASSETS = DECK / "assets"
...
def main() -> None:
    ...
    for line in scaffold:
        ...
        if m:
            ...
            out.append(line)
    (DECK / "harness.qmd").write_text("".join(out))
    print(f"wrote harness.qmd: {n_fb} flipbooks, {n_frames} figure slides")

    if "--render" in sys.argv:
        proc = subprocess.run(
            ["quarto", "render", "harness.qmd", "--to", "revealjs"],
            cwd=DECK,
            capture_output=True,
            text=True,
        )
