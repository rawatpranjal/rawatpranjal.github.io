"""Draw the inference/serving deck's true state: every figure is a
photograph of a real gpt2 forward pass, cache, or sampling step.

Design tokens are ported verbatim from the langchain deck's langviz.py
(itself ported from harnessviz.py / ragviz.py / gitviz.py), so all five
decks read as one system. The shared drawing primitives (pipeline, card,
scorecard, state, panels) are reused unchanged; the token strip, top-k bar,
KV-cache grid, and sampling-comparison functions are new to this deck.
...
canvas is what makes a growing token strip or filling cache grid read as
evolving rather than jumping.
"""
...
# ---- token strip (gpt2's byte-pair pieces, in order) ---------------------------
...
def draw_tokens(
    ...
    """A horizontal strip of token chips. `new` lights the just-generated
    chip(s) in NEW_COLOR; `ids` (optional) prints the real token id under
    each chip."""
    fig, ax = _new_fig(title, note)
    n = len(tokens)
    if not n:
    ...
    x0, x1 = 3.0, 97.0
    slot_w = (x1 - x0) / n
    box_w = min(slot_w * 0.86, 12.0)
    box_h = 8.0
    y_mid = 30.0
    for i, tok in enumerate(tokens):
        cx = x0 + slot_w * i + slot_w / 2
        is_new = i in new
        color = NEW_COLOR if is_new else NODE_EDGE
        ax.add_patch(
            FancyBboxPatch(
                (cx - box_w / 2, y_mid - box_h / 2),
                box_w,
                ...
                facecolor=NODE_FILL,
                edgecolor=color,
                linewidth=2.2 if is_new else 1.4,
            )
        )
...
# ---- top-k logits/probabilities (a real softmax over the vocab) ----------------
...
def draw_topk(
    ...
    x_label: str = "probability",
):
    """Horizontal bar chart over the top-k vocab entries, tallest first."""
    fig, ax = _new_fig(title, note)
    n = len(items)
    if not n:
        _save(fig, path)
    ...
    for i, (tok, v) in enumerate(items):
        y = y1 - (i + 1) * row_h
        is_pick = i == picked
        color = NEW_COLOR if is_pick else NODE_EDGE
        w = (x1 - x0) * (v / max_v)
        ax.add_patch(
            FancyBboxPatch(
                (x0, y + row_h * 0.16),
                ...
                facecolor=color,
                edgecolor="none",
                alpha=0.92 if is_pick else 0.62,
            )
        )
        ax.text(
            x0 - 1.5,
            y + row_h / 2,
            _short(tok.replace("Ġ", "␣"), 12),
            fontsize=7.4,
            family="monospace",
            color=INK,
            ha="right",
    ...
    ax.text(x0, y0 - 2.6, x_label, fontsize=7, color=MUTED)
    _save(fig, path)


# ---- KV-cache growth grid (layers x cached positions) ---------------------------
...
def draw_kv_cache(
    ...
    """A layers x positions grid: filled cells are cached keys/values,
    the newest column lit -- growth by one position per generated token."""
    fig, ax = _new_fig(title, note)
    cols = max(min(seq_len, max_cols), 0)
    x0, x1 = 22.0, 94.0
    y0, y1 = 8.0, 48.0
    n_layers = max(n_layers, 1)
    row_h = (y1 - y0) / n_layers
    col_w = (x1 - x0) / max(cols, 1)
    for layer in range(n_layers):
        ...
        for c in range(cols):
            ...
            ax.add_patch(
                FancyBboxPatch(
                    (x0 + c * col_w + col_w * 0.08, y + row_h * 0.1),
                    col_w * 0.84,
                    row_h * 0.8,
                    boxstyle="round,pad=0.04",
                    facecolor=NEW_COLOR if is_new else NODE_FILL,
                    edgecolor=NEW_COLOR if is_new else NODE_EDGE,
                    linewidth=1.6 if is_new else 1.0,
                    alpha=1.0 if is_new else 0.85,
                )
            )
    ax.text(
        x0,
        y1 + 2.0,
        f"K/V cache: {n_layers} layers x {cols} cached positions",
        fontsize=8,
        color=MUTED,
        fontweight="bold",
    )
...
# ---- sampling comparison (same logits, three strategies side by side) ----------
...
def draw_sampling(
    ...
    """Side-by-side strategy panels. Each panel is
    {'label': str, 'items': [(token, prob), ...], 'picked': int|None, 'stat': str}."""
    fig, ax = _new_fig(title, note)
    n = len(panels)
    if not n:
        _save(fig, path)
    ...
    x0, x1 = 3.0, 97.0
    gap = 2.4
    panel_w = (x1 - x0 - gap * (n - 1)) / n
    y0, y1 = 8.0, 50.0
    for i, p in enumerate(panels):
        px0 = x0 + i * (panel_w + gap)
        px1 = px0 + panel_w
        _panel_bg(ax, px0, px1, y0, y1, label=p["label"])
        items = p.get("items", [])
        picked = p.get("picked")
        m = len(items)
        if m:
            ...
            for j, (tok, v) in enumerate(items):
                ...
                is_pick = j == picked
                color = NEW_COLOR if is_pick else NODE_EDGE
                w = (panel_w - 2.0) * (v / max_v)
                ax.add_patch(
                    FancyBboxPatch(
                        (px0 + 1.0, y + row_h * 0.18),
                        max(w, 0.25),
                        row_h * 0.64,
                        boxstyle="round,pad=0.06",
                        facecolor=color,
                        edgecolor="none",
                        alpha=0.92 if is_pick else 0.58,
                    )
                )
                ax.text(
                    px0 + 1.2,
                    y + row_h / 2,
                    _short(tok.replace("Ġ", "␣"), 10),
                    fontsize=6.2,
                    family="monospace",
                    color=INK,
                    va="center",
...
if __name__ == "__main__":
    ...
    draw_tokens(
        ...
        out / "tokens.png",
        "Tokens: a sentence, split",
        new=(5,),
        ids=[464, 3797, 3332, 319, 262, 2603],
    )

    draw_topk(
        [("Ġmat", 0.41), ("Ġfloor", 0.11), ("Ġroof", 0.07), ("Ġchair", 0.05)],
        out / "topk.png",
        "Top-k: next-token distribution",
        picked=0,
    )

    draw_kv_cache(12, 6, out / "kv.png", "KV cache: growth after 6 tokens")

    draw_sampling(
        [
            {
                "label": "greedy",
                "items": [("Ġmat", 0.41), ("Ġfloor", 0.11)],
                "picked": 0,
                "stat": "argmax",
            },
            {
                "label": "temperature=1.4",
                "items": [("Ġmat", 0.28), ("Ġfloor", 0.19), ("Ġroof", 0.15)],
                "picked": 1,
                "stat": "H=1.8 nats",
            },
            {
                ...
                "items": [("Ġmat", 0.41), ("Ġfloor", 0.11)],
                "picked": 0,
                "stat": "kept 2 tokens, mass=0.92",
            },
        ],
        out / "sampling.png",
        "Sampling: same logits, three strategies",
    )

    draw_card(
        "paged attention + continuous batching\n-> higher GPU utilization at scale",
        out / "card.png",
        "Card: why vLLM is fast",
        tone="good",
    )

    draw_scorecard(
        [{"label": "fp16 bytes", "cells": ["62.3M"], "verdict": "pass"}],
        out / "scorecard.png",
        "Scorecard: quantization footprint",
        columns=["value"],
    )
    ...
    for p in pngs:
        assert p.stat().st_size > 15_000, (p.name, p.stat().st_size)
