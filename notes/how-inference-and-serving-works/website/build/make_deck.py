...
evolution of real gpt2 inference running on CPU. Everything else in the scaffold
passes through unchanged. Output is inference.qmd, which quarto renders to the
folder deck.

    /Users/pranjal/.venvs/inference-deck/bin/python build/make_deck.py [--render]
"""
...
DECK = HERE.parent  # notes/how-inference-and-serving-works/website
ASSETS = DECK / "assets"
...
def main() -> None:
    ...
    for line in scaffold:
        ...
        if m:
            ...
            out.append(line)
    (DECK / "inference.qmd").write_text("".join(out))
    print(f"wrote inference.qmd: {n_fb} flipbooks, {n_frames} figure slides")

    if "--render" in sys.argv:
        proc = subprocess.run(
            ["quarto", "render", "inference.qmd", "--to", "revealjs"],
            cwd=DECK,
            capture_output=True,
            text=True,
        )
