...
evolution of a real toy RAG pipeline. Everything else in the scaffold passes through
unchanged. Output is rag.qmd, which quarto renders to the folder deck.
...
def main() -> None:
    ...
    for line in scaffold:
        ...
        if m:
            ...
            out.append(line)
    (DECK / "rag.qmd").write_text("".join(out))
    print(f"wrote rag.qmd: {n_fb} flipbooks, {n_frames} figure slides")

    if "--render" in sys.argv:
        proc = subprocess.run(
            ["quarto", "render", "rag.qmd", "--to", "revealjs"],
            cwd=DECK,
            capture_output=True,
            text=True,
        )
